...
class TransactionObserver
{
    ...
    public function deleted(Transaction $transaction)
    {
        ...
        // Kembalikan stok produk dengan respek terhadap tipe stok yang digunakan
        foreach ($transaction->transactionItems as $item) {
            $product = $item->product;
            $quantityToReturn = $item->weight ?? $item->quantity;
            $stockType = $item->stock_type ?? 'regular';

            if ($stockType === 'kongsi') {
                $product->increment('stok_kongsi', $quantityToReturn);
            } else {
    ...
    public function restored(Transaction $transaction)
    {
        ...
        // Kurangi lagi stok dengan respek terhadap tipe stok
        foreach ($transaction->transactionItems()->get() as $item) {
            $product = $item->product;
            $quantityToDeduct = $item->weight ?? $item->quantity;
            $stockType = $item->stock_type ?? 'regular';

            if ($stockType === 'kongsi') {
                $product->decrement('stok_kongsi', $quantityToDeduct);
            } else {
